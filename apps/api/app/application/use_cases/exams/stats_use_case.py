...
from app.presentation.schemas.stats import ExamStatsOut, ExamSummary, ScoreDistributionItem, ParticipantStat, QuestionStat
...
class GetExamStatsUseCase:
    ...
    async def execute(self, exam_id: UUID) -> ExamStatsOut:
        ...
        for sc in user_best_scores.values():
            if sc < 2: dist["0-2"] += 1
            elif sc < 4: dist["2-4"] += 1
            elif sc < 6: dist["4-6"] += 1
            elif sc < 8: dist["6-8"] += 1
            else: dist["8-10"] += 1
