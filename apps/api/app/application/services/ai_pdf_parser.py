...
class AIPdfParserStrategy(IPdfParserStrategy):
    ...
    async def parse(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger.info(f"[{job_id}] ⏳ STEP 1: Running OpenDataLoader (Layout & Image Extraction)...")
            odl_start = time.time()
            ...
            logger.info(f"[{job_id}] ✅ OpenDataLoader finished in {time.time() - odl_start:.2f}s. Markdown length: {len(markdown_text)} chars.")
            
            # Dictionary to hold PIL images for Gemini Multimodal
            gemini_images = []
            
            # Clean markdown text from image tags to check actual text length
            import re
            text_without_images = re.sub(r'!\[.*?\]\(.*?\)', '', markdown_text)
            
            # If the PDF is scanned, the actual text will be very short or empty.
            is_scanned_pdf = len(text_without_images.strip()) < 100
            
            ocr = None
            if is_scanned_pdf:
                logger.warning(f"[{job_id}] ⚠️ SCANNED PDF DETECTED (Short text). Triggering Fallback...")
                logger.info(f"[{job_id}] ⏳ STEP 2: Loading & Running PaddleOCR on full-page images...")
                ocr_start = time.time()
                try:
                    ...
                    print(f"Failed to load PaddleOCR for scanned PDF fallback: {e}")
            ...
            if os.path.exists(image_dir):
                ...
                for img_file in os.listdir(image_dir):
                    if img_file.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        if is_scanned_pdf and ocr:
                            # SCANNED PDF FALLBACK: Extract text from full-page images
                            try:
                                result = ocr.ocr(img_path, cls=True)
                                if result and result[0]:
                                    for line in result[0]:
                                        markdown_text += line[1][0] + "\n"
                            except Exception as e:
                                logger.error(f"[{job_id}] PaddleOCR failed on {img_path}: {e}")
                        else:
                            # DIGITAL PDF: Let Gemini "see" the extracted images (Multimodal)
                            try:
                ...
                if is_scanned_pdf:
                    logger.info(f"[{job_id}] ✅ PaddleOCR finished in {time.time() - ocr_start:.2f}s.")
                else:
                    logger.info(f"[{job_id}] ✅ STEP 2: Digital PDF detected. Skipping OCR. Loaded {len(gemini_images)} images for Gemini.")
        ...
        if not api_key:
            logger.warning(f"[{job_id}] GEMINI_API_KEY is not set in settings. Skipping AI extraction.")
        ...
        try:
            ...
            for item in data:
                ...
                questions.append(ParsedQuestionPreview(
                    content=item.get("content", ""),
                    options=options,
                    solution=item.get("solution", "")
                ))
            
            logger.info(f"[{job_id}] 🎉 SUCCESS! Total time elapsed: {time.time() - total_start_time:.2f}s. Extracted {len(questions)} questions.")
            return questions
        except Exception as e:
