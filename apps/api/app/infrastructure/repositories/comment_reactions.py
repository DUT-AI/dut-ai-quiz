...
from app.infrastructure.persistence.models import CommentReaction, Comment
...
class CommentReactionRepository(ICommentReactionRepository):
    ...
    async def add_reaction(self, reaction: CommentReactionEntity) -> None:
        ...
        if reaction.reaction_type == ReactionType.like:
            stmt = update(Comment).where(Comment.id == reaction.comment_id).values(like_count=Comment.like_count + 1)
        else:
            stmt = update(Comment).where(Comment.id == reaction.comment_id).values(dislike_count=Comment.dislike_count + 1)
    ...
    async def remove_reaction(self, reaction: CommentReactionEntity) -> None:
        ...
        # Update counter
        if reaction.reaction_type == ReactionType.like:
            update_stmt = update(Comment).where(Comment.id == reaction.comment_id).values(like_count=Comment.like_count - 1)
        else:
            update_stmt = update(Comment).where(Comment.id == reaction.comment_id).values(dislike_count=Comment.dislike_count - 1)
        await self._session.execute(update_stmt)
        await self._session.flush()

    async def switch_reaction(self, reaction: CommentReactionEntity, old_type: ReactionType) -> None:
        stmt = update(CommentReaction).where(
            CommentReaction.comment_id == reaction.comment_id,
            CommentReaction.user_id == reaction.user_id
        ...
        if reaction.reaction_type == ReactionType.like: # Switching from dislike to like
            update_stmt = update(Comment).where(Comment.id == reaction.comment_id).values(
