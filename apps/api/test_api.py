...
async def test():
    ...
    app.dependency_overrides[get_current_user] = lambda: UserContext(id=17, roles=["student"])
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        print(
            "POST Response body:", response.json() if response.status_code == 200 else response.text
        )
