...
export default function PracticeLeaderboard({ lessonSlug, variant = "retro" }: PracticeLeaderboardProps) {
  ...
  if (isLoading) {
    return (
      <div className={`flex flex-col items-center justify-center py-10 ${isModern ? "font-sans" : "font-mono"}`}>
        ...
        <span className={`text-xs ${isModern ? "text-gray-navy dark:text-light-blue font-bold" : "text-zinc-500 dark:text-slate-400 font-extrabold"} animate-pulse`}>
  ...
  const containerClasses = isModern
    ...
    : "w-full bg-white dark:bg-slate-900 border-3 border-zinc-900 dark:border-slate-700 p-4 font-mono text-zinc-900 dark:text-slate-100 shadow-md";

  const headerClasses = isModern
    ? "flex items-center gap-2 border-b border-gray-150 dark:border-white/10 pb-3 mb-4"
    : "flex items-center gap-2 border-b-2 border-zinc-900 dark:border-slate-700 pb-2 mb-3";
  ...
  const tableHeaderClasses = isModern
    ...
    : "border-b border-zinc-200 dark:border-slate-800 text-zinc-500 dark:text-slate-400 font-extrabold";

  const tableRowClasses = isModern
    ? "hover:bg-gray-50 dark:hover:bg-white/5 transition-colors font-semibold"
    : "hover:bg-zinc-50 dark:hover:bg-slate-800/40 transition-colors font-bold";

  const tableBorderClasses = isModern
    ? "divide-y divide-gray-100 dark:divide-white/5"
    : "divide-y divide-zinc-100 dark:divide-slate-800/50";
  ...
  return (
    <div className={containerClasses}>
      ...
      {leaderboard.length === 0 ? (
        <div className={`text-center py-8 text-xs ${isModern ? "text-gray-navy dark:text-light-blue opacity-50 font-medium" : "text-zinc-400 dark:text-slate-500 font-bold"}`}>
            ...
            <tbody className={tableBorderClasses}>
              {leaderboard.map((row, idx) => {
                ...
                const modernRankColors = [
                  ...
                  "bg-slate-100 text-slate-655 dark:bg-slate-800/40 dark:text-slate-300 font-black",
                  ...
                  >
                    <td className="py-2 pr-2 text-center">
                      {isTop3 ? (
                        ...
                        <span className={isModern ? "text-gray-navy dark:text-light-blue opacity-70" : "text-zinc-500 dark:text-slate-400"}>
                          {idx + 1}
                        </span>
                      )}
                    </td>
                    <td className="py-2">
                      <div className="flex items-center gap-2">
                          ...
                          <img
                            ...
                            className={
                              isModern 
                                ...
                                : "size-5 rounded-none border border-zinc-900 dark:border-slate-700 object-cover"
                            }
                          />
                        ) : (
                          <div className={
                        ...
                        <span className={`truncate max-w-[120px] sm:max-w-none ${isModern ? "text-dark-blue dark:text-white" : "text-zinc-900 dark:text-slate-200"}`}>
                    ...
                    <td className={`py-2 text-right hidden sm:table-cell ${isModern ? "text-gray-navy dark:text-light-blue opacity-70" : "text-zinc-500 dark:text-slate-400"}`}>
